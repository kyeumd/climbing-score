...
export function viewScoreTable(ctx, gymId) {
  ...
  if (!gym) return h('div', {}, '짐을 찾을 수 없습니다.');
  ...
  if (grades.length === 0) {
    return h('div', { class: 'view' },
      ...
      panel(h('p', { class: 'subtitle' }, '난이도 색을 먼저 등록해야 점수표가 생깁니다.')),
    );
  }
    ...
    panel(
      ...
      h('div', { class: 'dials' },
        dial('기준 점수', table.baseScore, 10, LIMITS.baseScore,
          ...
          '내 레벨과 같은 난이도를 깼을 때 받는 점수'),
        dial('어려울 때 배수', table.upFactor, 0.1, LIMITS.upFactor,
          (v) => actions.setScoreTable(gymId, { upFactor: v }),
          `한 단계 위 문제마다 곱하는 값 (${LIMITS.upFactor.min}~${LIMITS.upFactor.max})`),
        dial('쉬울 때 배수', table.downFactor, 0.05, LIMITS.downFactor,
        ...
        h('div', { class: 'section-head', style: { marginBottom: '0.5rem' } },
          h('div', {},
            ...
            h('p', { class: 'hint', style: { marginTop: '0.35rem' } },
              '칸을 탭하면 직접 고칠 수 있습니다. 대각선이 기준 점수입니다.'),
          ),
...
function matrixTable(rows, grades, gymId, actions) {
  return h('div', {
    ...
    h('table', { class: 'matrix num' },
      h('thead', {},
        h('tr', {},
          h('th', { scope: 'col', class: 'matrix__corner' },
            h('span', { 'aria-hidden': 'true' }, 'LV \\ 난이도'),
      ...
      h('tbody', {},
        rows.map((row) => h('tr', {},
          h('th', { class: 'matrix__lv', scope: 'row' }, `LV${row.level}`),
          row.cells.map((cell, i) => h('td', {
            ...
            'aria-label': `LV${row.level} ${cell.grade.label} ${cell.score}점, 탭하면 수정`,
...
function editCell(cell, level, gymId, actions) {
  const input = prompt(`LV${level} × ${cell.grade.label} 점수`, String(cell.score));
